...
| GenerationRequest | Command | The input command that initiates a generation. Carries `requestId`, `userId`, `projectId`, `artifactType`, `model`, `input`, `toolKey`, `workflowType`, `idempotencyKey`, `outputFormat`, optional registry selectors, and optional step-level fields `briefingId`, `extractionArtifactId`, `stepDependencyArtifactIds`. In multi-step flows, each subsequent request must preserve and extend prior step context so outputs remain coherent with the requested `ToolStep`. | `frontend/src/features/generation/contracts/backend-stream.ts:4-21` | canonical |
...
| WorkflowRunMode | Value Object | Intent of a generation invocation relative to prior runs. `new` = no prior artifact, first-time generation; `resume` = continue from a paused or incomplete prior run; `regenerate` = re-run a step against an already-completed or failed prior artifact. | `src/lib/types/xstate.ts:22` | canonical |
...
Operational note for `ExtractionContext`: at ToolStep dispatch time, `GenerationRequest.input` must carry both non-empty `briefingText` and a structured `extractionPayload`, resolved deterministically from extraction artifact content (raw JSON, fenced JSON, envelope fields) with fallback to `sourceRequest.input.extractionPayload` (`frontend/src/features/tools/runtime/tools-client.ts:113-329`, `frontend/src/features/tools/ui/ToolPageTemplate.tsx:47-589`, `frontend/src/features/tools/runtime/tools-client.test.ts:134-187`).
