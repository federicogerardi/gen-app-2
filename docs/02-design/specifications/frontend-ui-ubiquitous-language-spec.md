...
- **Context generation trigger**: pre-step context assembly starts through one explicit setup CTA. During transition the visible copy can remain `Avvia estrazione`, but the action contract is `Start Context Generation Action` and can execute extraction, API-backed acquisition, and merge according to tool configuration.
