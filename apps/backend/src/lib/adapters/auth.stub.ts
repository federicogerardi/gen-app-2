...
export class AuthUserRepositoryStub implements AuthUserRepository {
  ...
  async createUser(input: CreateAuthUserInput): Promise<AuthUserRecord> {
    ...
    const record: AuthUserRecord = {
      ...
      monthlyCreditsUsed: input.monthlyCreditsUsed ?? 0,
      monthlyArtifactLimit: input.monthlyArtifactLimit ?? 1000,
      monthlyArtifactsUsed: input.monthlyArtifactsUsed ?? 0,
  ...
  async updateUser(userId: string, input: UpdateAuthUserInput): Promise<AuthUserRecord | null> {
    ...
    const updated: AuthUserRecord = {
      ...
      ...(input.monthlyCreditsUsed !== undefined ? { monthlyCreditsUsed: input.monthlyCreditsUsed } : {}),
      ...(input.monthlyArtifactLimit !== undefined ? { monthlyArtifactLimit: input.monthlyArtifactLimit } : {}),
      ...(input.monthlyArtifactsUsed !== undefined ? { monthlyArtifactsUsed: input.monthlyArtifactsUsed } : {}),
  ...
  if (options.seedSessions) {
    for (const seed of options.seedSessions) {
      void users.createUser({
        ...
        monthlyCreditsUsed: seed.user.monthlyCreditsUsed,
        monthlyArtifactLimit: seed.user.monthlyArtifactLimit,
        monthlyArtifactsUsed: seed.user.monthlyArtifactsUsed,
