...
> **ARCHIVED 2026-05-01** — Superseded da [feature-railway-same-origin-unified-1](../plan/feature-railway-same-origin-unified-1.md), piano unificato che integra questo piano con la strategia di migrazione Railway.
...
| Task | Description | Completed | Date |
| -------- | ----------- | --------- | ---- |
| TASK-005 | Configurare il layer di routing same-origin su Railway aggiornando `railway.toml` del servizio frontend per aggiungere le direttive di proxy verso il backend Railway, oppure introducendo un servizio proxy dedicato nel progetto Railway, e committare la configurazione nel repository. |  |  |
...
- **FILE-003**: Configurazione Railway da aggiornare in `frontend/railway.toml` (o servizio proxy dedicato nel progetto Railway) per dichiarare le regole di routing same-origin.
