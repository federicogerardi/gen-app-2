...
| `IssuePublicationPolicy` | Frontend/UI | Frontend/UI | Only `UserReportCategory = issue` is eligible for GitHub publication. Categories `feature-request` and `other` must remain local backlog items. See DDD-065. |
...
| `IssuePublicationPolicy` escalation gate | Frontend/UI | GitHub publication is allowed only when `UserReport.category = issue`. External publication failure must not delete local `UserReport`; status remains local and recoverable (`submitted` or `triaged`) until escalation succeeds. | DDD-065 |
