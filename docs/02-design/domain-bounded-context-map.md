...
| `ToolWorkflow` / `ToolKey` | Generation ↔ Frontend/UI | both | `ToolKey` is the cross-context canonical identifier for Tool identity (DDD-029). At the Generation ↔ Frontend/UI boundary: `SupportedTool` (Frontend, kebab-case) is passed as the `toolKey` field in `GenerationRequest` — no value transformation required. Canonical values include `funnel-pages`, `nextland`, `youtube-lf-script`, `angle-generator`, and the approved reactivation identity `meta-ads` (DDD-094). `ToolWorkflow` (Generation, snake_case, DB-compatible) is derived independently for artifact routing and is not the same concept as `ToolKey`; canonical values include `funnel_pages`, `nextland`, `youtube_lf_script`, `angle_generator`, `meta_ads_generator`, and `extraction` route-type (DDD-040, DDD-077, DDD-094). `meta_ads` is deprecated as a legacy alias and must not be used as a primary workflow identity (DDD-094, superseding DDD-030). Convention divergence between kebab and snake_case is an explicit, resolved translation rule (DDD-C-005, resolved-documented). See DDD-029, DDD-030, DDD-040, DDD-077, DDD-094. |
