...
**Key Value Objects**: `ToolPageViewModel`, `ReadinessSnapshot`, `ReadinessReasonCode`, `CanonicalToolUiState`, `PrimaryActionPolicy`, `SecondaryActionFlags`, `SupportedTool`, `ToolStep`, `ToolStepStatus`, `BriefingFile`, `ExtractionContext`, `HydrationResult`, `GenerationArtifact`, `LlmModelSelector`  
...
| `ExtractionContext` schema (youtube-lf-script) | Generation | Frontend/UI | For `youtube-lf-script`, canonical extraction fields are `knowledge_content`, `avatar`, `pain_point`, `purchase_process_type`, `offer`, `proof`, `tone`, `target_duration_minutes`, `proprietary_methodology_disclosure`. Missing fields are normalized to `null` (no inferred defaults). See DDD-042. |
...
| `ToneProfile` | Frontend/UI | Generation | **Provisional** — Frontend currently forwards `GenerationRequest.input.tone` as an optional passthrough string sourced from entry parameters (`?tone=`). No constrained enum validation or dedicated ToolPage tone selector is enforced yet. Target governance remains tracked in DDD-039. |
| `ArtifactRelaunch` | Frontend/UI | Generation | Entering a tool from an existing artifact must resolve `HydrationResult` by `ArtifactType`: direct hydration for `extraction`, linked extraction lookup for `content` (via `briefingId`/`extractionArtifactId`). Readiness completeness is text-first per DDD-038: hydration is complete when the recovered context includes `extractionArtifactId`, `briefingId`, and non-empty normalized briefing text. `extractionPayload` is optional passthrough for dispatch (DDD-021), not part of the readiness gate. After hydration, Frontend exposes one effective generation-start action (`start-generation`) and applies default runtime intent `regenerate` for artifact-driven relaunch entries. See DDD-038. |
...
| `ToneProfile` dispatch handling (provisional) | Frontend/UI → Generation | Current behavior forwards `GenerationRequest.input.tone` as an optional passthrough string for generation and regenerate dispatches. No enforced constrained value set or dedicated ToolPage selector is implemented yet. The constrained-value governance target remains tracked as provisional in DDD-039. | DDD-039 |
