...
type: code-review
tags: [architecture, code-review, weaknesses, technical-debt]
---
...
Severity-first ranking of active findings identified in the 2026-05-26 refresh. All findings are evidence-based with direct anchor paths.
...
- Generation flow completion remains partially dependent on Frontend/UI liveness signals.
  - Impacted concepts: `GenerationSession`, `WorkflowStepOrchestration`, `WorkflowStepCompleted`.
  - Why weak: if the client disconnects or the Tool Workspace is interrupted, session completion can remain tied to FE-driven progression instead of a backend-owned continuation path.
  - Review trace for future implementation: introduce a backend-owned session batch continuation job that can resume and complete eligible `GenerationSession` runs without requiring FE presence, with deterministic idempotency and replay-safe status transitions.

- Tool Workspace can remain stuck in generation mode after FE/runtime interruptions.
  - Impacted concepts: `CanonicalToolUiState` (`running`), `DispatchError`, `ReadinessSnapshot`.
  - Why weak: interruption/error scenarios can leave the page in a non-recoverable running state with insufficient unblock feedback for the user.
  - Review trace for future implementation: add a deterministic fail-close recovery policy that forces transition out of `running`, surfaces actionable `DispatchError`, and exposes explicit user recovery actions (retry/reset/reload-session) without requiring manual page workarounds.

### LOW

- No open LOW findings.

## Evidence Refresh Delta (2026-05-26)

### New Future-Implementation Traces Recorded
- Added review trace: backend-owned batch continuation for `GenerationSession` completion to reduce dependency on FE liveness during long-running flows.
- Added review trace: Tool Workspace fail-close recovery for FE interruption/error cases to prevent sticky generation mode without user escape path.
...
- **Hydration Non-Determinism vs. Requested Briefing is CLOSED** (executed 2026-05-20, DDD-075 enforcement complete):
  ...
  - Closure evidence anchors: `apps/backend/src/lib/runtime/auth-http/tools-hydrate-handlers.ts`, `apps/backend/src/lib/runtime/auth-http/support.ts`, `apps/backend/src/lib/tests/runtime.auth-http.test.ts`, `../05-plans/process-hydration-briefing-coherence-finding-closure-1.md`.
...
- Generation orchestrator monolith finding is closed: the previous single-file machine definition has been decomposed into typed helper modules and state fragments, with `generation-system.definition.ts` reduced to thin root composition (`setup + context + states spread`). Closure evidence: plan `../05-plans/refactor-generation-system-definition-1.md` updated to `Completed` on 2026-05-19, normalized LOC gate passed (`definition=47`, each extracted module <= 300), and regression gates passed (`typecheck`, runtime test suite, integration suite, backend full suite).
