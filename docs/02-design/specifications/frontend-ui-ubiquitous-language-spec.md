...
| Missing Required Files Message | Blocking setup feedback listing policy-required files that must be uploaded to enable primary generation action. | Tool Workspace Setup Panel, directly above primary CTA | Generic upload warning without required list |
| Missing Optional Files Advisory | Non-blocking informational recommendation shown when optional files are missing but required files are complete. | Tool Workspace Setup Panel + optional inline file rows | Warning/error feedback that blocks CTA |
