...
- [repository-publication-cleanup-1](./03-development/plans/repository-publication-cleanup-1.md) - planned
- [refactor-frontend-backend-dead-code-drift-1](./03-development/plans/refactor-frontend-backend-dead-code-drift-1.md) - in progress (Sprint 5 / 6 pending)
...
- **Hotspot 1 — Contract duplication FE/BE**: `GenerationRequest` e `BackendStreamEvent` risultano duplicati tra [frontend/src/features/generation/contracts/backend-stream.ts](../apps/frontend/src/features/generation/contracts/backend-stream.ts) e [src/lib/runtime/request-contract.ts](../apps/backend/src/lib/runtime/request-contract.ts) / [src/lib/runtime/stream-contract.ts](../apps/backend/src/lib/runtime/stream-contract.ts). Primo controllo consigliato: drift di campi opzionali (`briefingId`, `extractionArtifactId`, `stepDependencyArtifactIds`) e compatibilità parser SSE.
- **Hotspot 2 — Legacy compatibility layer**: presenza di shim e alias deprecati ancora attivi in [frontend/src/features/tools/runtime/tool-ux-state.ts](../apps/frontend/src/features/tools/runtime/tool-ux-state.ts), [frontend/src/features/tools/runtime/tool-form-architecture.ts](../apps/frontend/src/features/tools/runtime/tool-form-architecture.ts), [frontend/src/features/tools/machines/tool-page.machine.ts](../apps/frontend/src/features/tools/machines/tool-page.machine.ts). Priorità: classificare in uso runtime vs solo test per preparare piano di rimozione per ciclo.
- **Hotspot 3 — Endpoint and capability mismatch**: il frontend dichiarava path admin aggiuntivi in [frontend/src/app/runtime/api-paths.ts](../apps/frontend/src/app/runtime/api-paths.ts) (`/api/admin/models`, `/admin/activity`), mentre il backend runtime gestisce esplicitamente route diverse. Risolto in Sprint 3.
...
- **✅ Bug risolto — listing projects/artifacts vuoti dopo navigazione SPA**: `BackendCapabilities` per `projects` e `artifacts` defaultavano a `false` (opt-in) → query hooks non eseguivano fetch. Corretto in [backend-capabilities.ts](../apps/frontend/src/app/runtime/backend-capabilities.ts): `projects` e `artifacts` ora opt-out (abilitati di default quando env var assente). Stabilizzata identity di `capabilities` in [AuthSessionProvider.tsx](../apps/frontend/src/app/providers/AuthSessionProvider.tsx) via `useMemo` per prevenire loop di re-fetch.
