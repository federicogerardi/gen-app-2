...
| WorkflowStep | Entity | A single named step within a multi-step tool generation flow. Exists in two forms: `WorkflowStepDescriptor` (static configuration: name, type, dependencies) and `WorkflowStepState` (runtime tracking: status, artifact id). Backend abstract descriptor — frontend concrete counterpart is `ToolStep`. Step dependencies enforce progressive context accumulation across steps. | `src/lib/types/xstate.ts` (`WorkflowStepDescriptor`, `WorkflowStepState`) | canonical |
